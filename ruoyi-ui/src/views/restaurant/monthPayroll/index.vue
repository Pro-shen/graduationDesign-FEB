    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
            ...
            <el-form-item label="创建时间">
                <el-date-picker v-model="dateRange" style="width: 240px" value-format="yyyy-MM-dd" type="monthrange"
                    range-separator="-" start-placeholder="开始月期" end-placeholder="结束月期"></el-date-picker>
            </el-form-item>
            <el-form-item>
        ...
        <el-row :gutter="10" class="mb8">
            <el-col :span="1.5">
                <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                    v-hasPermi="['restaurant:monthPayroll:add']">新增</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
                    v-hasPermi="['restaurant:monthPayroll:edit']">修改</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
                    v-hasPermi="['restaurant:monthPayroll:remove']">删除</el-button>
            </el-col>
            <el-col :span="1.5">
                <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                    v-hasPermi="['restaurant:monthPayroll:export']">导出</el-button>
            </el-col>
            <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>

        <el-table v-if="refreshTable" v-loading="loading" :data="tPayrollList" row-key="id"
            ...
            <el-table-column :formatter="formatterTime" prop="createTime" label="月期" width="200"></el-table-column>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.parentId != 0" size="mini" type="text" icon="el-icon-delete"
                        @click="handleDelete(scope.row)" v-hasPermi="['restaurant:monthPayroll:remove']">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="工资管理" :visible.sync="open" width="500px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="100px">
                ...
                <el-form-item label="工资月期" prop="createTime">
                    <el-date-picker v-model="form.createTime" value-format="yyyy-MM-dd" type="month" placeholder="选择月期">
                    </el-date-picker>
                </el-form-item>
            </el-form>
...
<script>
import { listTPayroll, add, selectTPayrollListById, edit, remove } from "@/api/restaurant/monthPayroll"
    ...
    methods: {
        ...
        handleDelete(row) {
            ...
            for (var i = 0; i < ids.length; i++) {
                if (i == ids.length - 1) {
                    str = str + "用户名为:" + userNames[i] + ",月期为:" + createTimes[i]
                } else {
                    str = str + "用户名为:" + userNames[i] + ",月期为:" + createTimes[i] + ","
                }
            }
            if (str == "") {
                listOrInt = 0
                str = "用户名为:" + userNames + ",月期为:" + createTimes
            }
                ...
                remove(data).then(res => {
                    if (res.data.ids.length == 0) {
                        ...
                        for (i = 0; i < res.data.ids.length; i++) {
                            resStr = resStr + "用户名为:" + res.data.ids[i].userName + ",月期为:" + res.data.ids[i].createTime + ",";
                        }
        ...
        handleExport() {
            this.download('/restaurant/monthPayroll/export', {
                ...this.queryParams
            }, `月工资管理_${new Date().getTime()}.xlsx`)
        },
        handleQuery() {
                    ...
                    if (this.add) {
                        add(this.form).then(res => {
                            if (res.data == -2) {
                                ...
                                this.$modal.msgError("该月期下已有用户及其工资信息存在")
                            } else if (res.data == 0) {
